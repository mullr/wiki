This work is inspired by the article `De Bruijn Notation as a Nested Datatype`_ by Richard Bird and Ross Paterson. Journal of Functional Programming, 9(1):77-91, January 1999. 
...
This definition does not work because the occurence of ``Term`` inside ``(Maybe (Term A))`` doesn't meet Coq's PositivityRequirement_.  And even if it did ``(Term (Maybe (Term A)))`` would probably require ImpredicativeSet_.
...
This is also known as the adbmal calculus. See DimitriHendriks_.
...
.. _De Bruijn Notation as a Nested Datatype: http://www.soi.city.ac.uk/~ross/papers/debruijn.html

.. _PositivityRequirement: ../PositivityRequirement

.. _ImpredicativeSet: ../ImpredicativeSet

.. _DimitriHendriks: ../DimitriHendriks
