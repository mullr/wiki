...
But note that using this https protocol might not work out-of-the-box due to issues with the ssl certificate of gforge.inria.fr, more details in the [gforge FAQ](http://siteadmin.gforge.inria.fr/FAQ.html#git_https). A workaround is to set the environment variable "GIT\_SSL\_NO\_VERIFY=true" in your shell, at least during the git operations.
...
For memory, here are the main steps used during the transition. More information on the process in the documentation and [FAQ](http://siteadmin.gforge.inria.fr/FAQ.html) of gforge.inria.fr.
