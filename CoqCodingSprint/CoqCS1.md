...
`attachment:schedule-coqcs1.pdf`_Tentative schedule`attachment:None`_, `attachment:intro-talk.pdf`_Slides`attachment:None`_ for the introduction
...
#. post a `message with subject REGISTER`_.  In case you already have an project in mind, please include a short description of it in the registration email.
...
`attachment:itinerary-beach-coqcs1.pdf`_Itinerary`attachment:None`_
...
* coqide:
  ...
  #. Make coqide silently kill coqtop and save the current buffer position when too many tabs are opened and restart coqtop when the user re-opens an old tab. (see 4074_)

  #. Remove use of threads in the coqide back end: use CPS+Unix.select to code a cooperative threads library (in the LWT style), see tentative patch `by PMP`_.
...
* tactics
  ...
  1. CUPS like syntax for selecting a set of goals for tactic application.  I.e. generalize "all:" to "1,2-5:", see also 1805_.
...
* OSX
  ...
  #. see if an opam root (compressed, to be unpacked in /opt/coq$V/opam/ on the fly) can be integrated in the .app bundle and have the bundled CoqIDE use that coqtop
...
* improving Search:
  ...
  #. Invent a "search" that works for tactics: search the patterns used in the tactic, or use patterns given by the user. (`related work`_)
...
* tactics:
  ...
  #. Coding the tactics of the ``logic.ml`` file in the new proof engine. These were the primitive tactics of the old proof engine and are currently implemented using a compatibility layer which, in particular, makes the ``Info`` command unaware of their semantics.

  #. Implement a "debug" trace for tactics. The ``Info`` command gives a trace of the tactics which were effectively applied. In order to debug scripts, it may be useful to also have a trace of all the tactics which were attempted.

  #. Port LtacProf_ to Coq 8.5, possibly hooking into the ``Info`` infrastructure.
...
* (bonus) Development benchmark for Coq developers
  ...
  * It might be useful to also keep track of little patches that may need to  be applied to the development, in order to cope with non-backward compatible  changes. Such patches could be commited in the git of the Coq developers.
  ...
  .. _here: https://coq.inria.fr/cocorico/CoqCodingSprint/CoqCS1/log

  .. _coordination mailing list: https://sympa.inria.fr/sympa/info/coq-coding-sprint

  .. _message with subject REGISTER: mailto:coq-coding-sprint@inria.fr?subject=REGISTER

  .. _relatively simple bugs: https://coq.inria.fr/cocorico/CoqCodingSprint/CoqCS1/bsp

  .. _bug triaging: https://coq.inria.fr/bugs/

  .. _4074: https://coq.inria.fr/bugs/show_bug.cgi?id=4074

  .. _by PMP: https://github.com/ppedrot/coq/commits/thread-free

  .. _1805: https://coq.inria.fr/bugs/show_bug.cgi?id=1805

  .. _Win64 installer: https://ci.inria.fr/coq/job/coq-win64-8.5/label=coq-win7-64/lastSuccessfulBuild/artifact/dev/nsis/

  .. _`coq:shell`: https://github.com/coq/opam-coq-shell

  .. _#3904: https://coq.inria.fr/bugs/show_bug.cgi?id=3904

  .. _related work: http://staff.computing.dundee.ac.uk/katya/ML4PG/

  .. _Bug 3934: https://coq.inria.fr/bugs/show_bug.cgi?id=3934

  .. _github: https://github.com/wjzz/Coq-Developers-Manual

  .. _LtacProf: http://www.ps.uni-saarland.de/~ttebbi/ltacprof/

  .. _Bug 4110: https://coq.inria.fr/bugs/show_bug.cgi?id=4110
