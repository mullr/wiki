...
* (Assia, with the help of Pierre B.) Learnt about _CoqProject_ (with ProofGeneral_, to handle plugins, options...). Started rebasing old code on v8.5.
...
* (Thomas, with help from Hugo H. and Pierre B.) Move fatal_error to the Errors module (a110da8_), and then implemented a printer in cbv and cbn (dc3f2ed_).
...
* (Jason, with help from Arnaud) Ported LtacProf_ to Coq 8.5; it compiles, but there seem to be heisenbugs, and other strange behavior.  (With help from Hugo) Started reverting 18796b6_ to add ``function_scope``, fixing `#3080`_
...
* (Gabriel) report a Merlin issues affecting Coq plugin authors: `merlin#410`_, `merlin#411`_. Elisp parser for Anomaly stack traces: `#78`_.
...
* (Abhishek) ROSCoq_ failed to compile with the `8.4 tip`_. Minimized the problem with Jason and filed a `bug report`_. Patched ROSCoq with Jason's workaround and got it to compile. Then extracted the `robotic program`_ to Haskell. The Haskell extract failed to compile. Manually fixed_ the extract to make it compile. It produced `correct results`_ when run. Hence, decided that the main way to run ROSCoq programs will be to extract them to Haskell and then to link them somehow with the roshask_ bindings for the `Robot Operating System`_.

* (Assia, with Pierre B.) still rebasing... plus some more discussions about _CoqProject_. Started editing its documentation on the reference manual.
...
* (Jason) Added back [Bind Scope ... with Sortclass], [Bind Scope ... with Funclass], added [function_scope] declared in Coq.Init.Notations, moved [type_scope] into user-space, rather than special-casing it in OCaml (the string ["type_scope"] no longer appears anywhere in the Coq source code).  Submitted a `pull request`_.
...
* (Jason)
  ...
  * Fixed `bug #4262, Output of [Print Scopes] is missing class keys`_
  ...
  * `Implemented most of the [Trace] vernacular`_, currently stuck on finding a way of `comparing lazy data types for strict equality that isn't [Pervasives.(=)] (which doesn't like functions)`_
...
* (Abhishek) Studied roshask_. Used it to generate Haskell definitions of some ROS_ message types (e.g. Twist_). Realized that it should be easy to tweak roshask to generate corresponding Coq definitions as well, e.g. change *data* to *Inductive*. The generated Coq file should also include Extraction directives to extract Coq Message types to the corresponding Haskell versions. The Haskell definitions (of message types) also includes extra functions, e.g. for (de) serialization. These would not be included in the Coq version.
...
* (Assia) Still trying to have the generic-equality branch compile. Edition of the _CoqProject_ / coq_makefile section of the reference manual.
...
.. _a110da8: https://github.com/tomsib2001/coq/commit/a110da85ea25ba6b1f5a4001142abf200da3f193

.. _dc3f2ed: https://github.com/tomsib2001/coq/commit/dc3f2ed953979a965a48ace94c3c344e5e9d0c7e

.. _LtacProf: www.ps.uni-saarland.de/~ttebbi/ltacprof/

.. _18796b6: https://github.com/coq/coq/commit/18796b6aea453bdeef1ad12ce80eeb220bf01e67

.. _#3080: https://coq.inria.fr/bugs/show_bug.cgi?id=3080

.. _robotic program: http://www.cs.cornell.edu/~aa755/ROSCoq/coqdoc/ROSCOQ.icreateConcrete.html#robotProgramInstance

.. _8.4 tip: https://github.com/coq/coq/commit/607b1cbfc819c321c1f5449ee10d12789f51e09a

.. _merlin#410: https://github.com/the-lambda-church/merlin/issues/410

.. _merlin#411: https://github.com/the-lambda-church/merlin/issues/411

.. _#78: https://github.com/coq/coq/pull/78

.. _ROSCoq: http://www.cs.cornell.edu/~aa755/ROSCoq/

.. _bug report: https://coq.inria.fr/bugs/show_bug.cgi?id=4260

.. _fixed: https://github.com/aa755/ROSCoq/commit/972ed9d26ac642499e60098cf0510ee9b6e0d3ea#diff-f9928ac6597fcd7e0cfbf7e9da0cdba9R2376

.. _correct results: https://github.com/aa755/ROSCoq/commit/972ed9d26ac642499e60098cf0510ee9b6e0d3ea

.. _roshask: https://github.com/acowley/roshask

.. _Robot Operating System:
.. _ROS: http://www.ros.org/

.. _pull request: https://github.com/coq/coq/pull/77

.. _bug #4262, Output of [Print Scopes] is missing class keys: https://coq.inria.fr/bugs/show_bug.cgi?id=4262

.. _Implemented most of the [Trace] vernacular: https://github.com/JasonGross/coq/tree/debug-command

.. _comparing lazy data types for strict equality that isn't [Pervasives.(=)] (which doesn't like functions): https://github.com/JasonGross/coq/blob/1a64808ad4b2a840c2cb115ae2e90cb632ac4f7e/engine/proofview_monad.ml#L105

.. _Twist: http://docs.ros.org/api/geometry_msgs/html/msg/Twist.html
