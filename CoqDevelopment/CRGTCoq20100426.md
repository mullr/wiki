...
==== Representation actuelle des entiers machines ====
   ...
   Le compilateur et la vm represente cet inductif (quand tout les arguments sont clos) directement par un entier machine.
   Mais pas le kernel ==> 
   ...
   Oui, mais on peut les partager (mais le type checker ne gere pas le partage).
   De plus substitution dans le type non-dependant => introduire un produit non dependant?
...
Decision d'ajouter les entiers natifs et les tableaux directement dans le kernel.
  ...
  Avantages esperes:
     ...
     - Representation plus compacte de certains termes (typiquement les certificats des preuves reflexives).
...
==== Solution mise en place (travail en cours) ====
   ...
   Les primitives sur ces types sont des definitions globales:
       - Sans corps (des parametres) mais qui ont des regles de reecritures associees.
...
==== Modification des fonctions de reductions ====
   Il y a plusieurs fonctions de reduction utilisant differentes representation des termes (lazy, cbv, vm).
   Mise en place, d'un module foncteur generique pour la reduction des primitives.
...
==== Des questions pour vous ====
   ...
   - Beaucoup de representation des termes: constr, fconstr, raw_constr, constr_pattern ... ou s'arreter?
...
==== Declaration des primitives ====
    ...
    Question: 
       - Le systeme doit-il connaitre directement les primitives et leur type
       - Si oui comment l'implementer?
       - Probleme avec le type de certaines primitives: besoin de le definition de certain type inductif: bool, pair, carry

==== Notations ====
   - Pour les entiers pas vraimment de discution.
   - Notations pour les tableaux: [| def | v0; ...; vn|]
