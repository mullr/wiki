...
This tutorial uses unicode characters and hence requires the UTF8Module_.

::

   Require Import utf8.

The first step is to write a ``Module Type`` in a file that contains the signature of the abstract structure to work from.  This will be the signature of a decidable total order.  A ``Module Type`` is just a listing of ``Parameter``s and ``Axiom``s.  In this case we also add a Notation_ to make the syntax nicer.
...
.. _UTF8Module: ../UTF8Module

.. _Notation: ../Notation

.. _DotNotation: ../DotNotation

.. _Qpositive: http://coq.inria.fr/contribs/QArith-Stern-Brocot.html
